...
from mirrored_supModel.BABELdata import rotation_conversions as geometry
...
def quat_fk(lrot, lpos, parents):
    ...
    :param lrot: tensor of local quaternions with shape (..., Nb of joints, 4)  局部四元数 B, F, J, 4
    :param lpos: tensor of local positions with shape (..., Nb of joints, 3)    局部位置  B, F, J, 3
...
def quat_fk_cuda(lrot, lpos, parents):
    """
    Performs Forward Kinematics (FK) on local quaternions and local positions to retrieve global representations
    在局部位置和局部四元数上执行FK以求全局表示 全局位置和全局旋转
    :param lrot: tensor of local quaternions with shape (..., Nb of joints, 4)  局部四元数 B, F, J, 4
    ...
    for i in range(1, len(parents)):
        # gp.append(quat_mul_vec(gr[parents[i]], lpos[..., i:i+1, :]) + gp[parents[i]])   # quat_mul_vec
        # gr.append(quat_mul(gr[parents[i]], lrot[..., i:i+1, :]))
        gp.append(geometry.quaternion_apply(gr[parents[i]], lpos[..., i:i+1, :]) + gp[parents[i]])
        gr.append(geometry.quaternion_multiply(gr[parents[i]], lrot[..., i:i+1, :]))
...
def interpolate_local(lcl_r_mb, lcl_q_mb, n_past, n_future):
    ...
    LERP is performed on the root's positions. 在根关节的位移上执行LERP
    ...
    :return: Interpolated root and quats 返回插值的根关节位移和四元数
    inter_lcl_r_mb
    inter_lcl_q_mb
    """
    # Extract last past frame and target frame  使用最近的两帧  帧下标分别为 n_past - 1 和 -n_future
    start_lcl_r_mb = lcl_r_mb[:, n_past - 1, :, :][:, None, :, :]  # (B, 1, 1, 3)
    end_lcl_r_mb = lcl_r_mb[:, -n_future, :, :][:, None, :, :]  # B, 1, 1, 3

    start_lcl_q_mb = lcl_q_mb[:, n_past - 1, :, :]  # B, 1, J, 4
    end_lcl_q_mb = lcl_q_mb[:, -n_future, :, :]     # B, 1, J, 4

    # LERP Local Positions:
    n_trans = lcl_r_mb.shape[1] - (n_past + n_future)   # T-(n_past + n_future) 需要插值的帧数？？ 为什么不把它作为一个参数传进来？
    ...
    const_trans = np.tile(start_lcl_r_mb, [1, n_trans + 2, 1, 1])    # B, n_trans + 2, 1, 3
    ...
    # inter_lcl_r_mb 根关节的位移插值结果 B, n_trans + 2, 1, 3 包含了给定的起始帧和结束帧在内的两帧
